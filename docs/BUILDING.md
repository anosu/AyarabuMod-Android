...
项目入口：`src/AyarabuMod/AyarabuMod.csproj`。标准方案：`AyarabuMod-Android.slnx`。
...
打开生成的 `AyarabuMod-Android.local.slnx`。已有本地覆盖不应被模板覆盖。CI 和 `UsePinnedSharedDependencies=true` 总是使用固定源码；标准 VS 方案也忽略本地覆盖。无需复制共享 DLL。
