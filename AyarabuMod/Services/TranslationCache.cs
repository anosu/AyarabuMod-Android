...
public sealed class TranslationCache
{
        ...
        try
        {
            ...
            if (local != null)
            {
                ...
                if (type != TranslationPaths.Manifest)
                    Toast.Warning(
                        "翻译服务",
                        $"「{ResourceLabel(type, id)}」无法更新，使用本地缓存"
                    );
                _memory[memoryKey] = local;
                return local;
            }
            ...
            if (type != TranslationPaths.Manifest)
                Toast.Error(
                    "翻译服务",
                    $"「{ResourceLabel(type, id)}」加载失败，保留原文，稍后可重试"
                );
            return null;
        }
        finally
    ...
    private static string ResourceLabel(string type, string? id) =>
        type == TranslationPaths.Names ? "人物名" : $"剧情 {id}";
    ...
    private static bool MatchesHash(string? json, string expectedHash)
    {
        try
        {
            return json != null && TranslationHash.Compute(json) == expectedHash;
        }
